...
search_with_zoxdie() {
    if [ -z "$1" ]; then
        ...
        file="$(fd --type f -I -H -E .git -E .git-crypt -E .cache -E .backup | fzf --height=70% --preview='bat -n --color=always --wrap=auto --tabs=0 --style=rule,numbers,snip,changes,header,header-filesize --line-range :500 {}' --preview-window=60%)"
        if [ -n "$file" ]; then
            nvim "$file"
        fi
    else
        ...
        if [ -n "$lines" ] && [ "$line_count" -eq 1 ]; then
            ...
            file=$(echo "$lines" | fzf --query="$1" --height=70% --preview='bat -n --color=always --wrap=auto --tabs=0 --style=rule,numbers,snip,changes,header,header-filesize --line-range=:500 {}' --preview-window=60%)
            if [ -n "$file" ]; then
                nvim "$file"
            fi
        else
