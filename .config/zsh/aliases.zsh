...
alias zsh-cleanup='source ~/scripts/cleanup-zwc.zsh'
# alias zsh-cleanup2='source ~/scripts/cleanup-zwc-bak.zsh'
...
if command -v fd &> /dev/null; then
    ...
    alias rmDS='fd -HI --type f --glob ".DS_Store" --exec rm -v {} + 2>/dev/null; fd -HI --type f --glob "._.DS_Store" --exec rm -v {} + 2>/dev/null'
else
...
if command -v brew &> /dev/null; then
    alias bbdump="brew bundle dump --describe --file=~/gen_brewfile"  # Generate a brewfile with descriptions
    alias buubi="brew update && brew upgrade && brew bundle install"  # Main Command (after adding to brewfile)
    alias bbcuz="brew bundle cleanup --zap"  # Cleanup unneeded packages aggressively
...
if command -v tmux &> /dev/null; then
    ...
    alias tns="~/scripts/tmux-sessionizer" # called from ~/scripts/
fi
